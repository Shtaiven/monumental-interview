...
robot_client::Input controller(const robot_model::RobotModel &model,
    ...
    // Simplest is to first rotate towards the setpoint, then drive forward towards it
    ...
    double target_theta = std::atan2(distance_vec.y, distance_vec.x + 1e-6); // add a small diff to prevent divide by 0
    double theta_diff = target_theta - current_theta;
    if (std::abs(theta_diff) > 0.05) {
        // Rotate towards target
        double angular_velocity = std::max(0.0, std::min(2.0, std::abs(theta_diff) * 10.0));  // clamp [0, 2]
        if (theta_diff < 0) {
    ...
    if (distance > 1e-6) {
        ...
        double linear_velocity =
            std::max(0.0, std::min(2.0, distance * 2.0));  // clamp [0, 2]
