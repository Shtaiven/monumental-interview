...
void ParticleFilter::predict(double dt, double gyro_z, double acc_x, double acc_y, double std_motion[3]) {
    ...
    for (auto& p : particles_) {
        ...
        double ax_global = std::cos(p.theta) * acc_x - std::sin(p.theta) * acc_y;
        double ay_global = std::sin(p.theta) * acc_x + std::cos(p.theta) * acc_y;
...
void ParticleFilter::updateGPS(double gps_x, double gps_y, double std_gps[2]) {
    ...
    for (auto& p : particles_) {
        ...
        // Gaussian likelihood
        double weight = std::exp(-0.5 * (dx*dx/(std_gps[0]*std_gps[0]) + dy*dy/(std_gps[1]*std_gps[1])));
        p.weight = weight;
        sum_weights += weight;
    }
    // Normalize weights
